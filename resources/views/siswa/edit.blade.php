        ...
        <div class="card border-0 shadow-sm rounded-3">
            ...
            <div class="card-body p-4">
                <form action="{{ route('siswa.update', $siswa->id_siswa) }}" method="POST">
                    ...
                    <!-- NISN (maxlength="10" dihapus) -->
                    <div class="mb-3">
                        <label for="nisn" class="form-label fw-medium">NISN <span class="text-danger">*</span></label>
                        <input type="text" name="nisn" id="nisn" class="form-control @error('nisn') is-invalid @enderror" value="{{ old('nisn', $siswa->nisn) }}" placeholder="Masukkan NISN" required>
                        @error('nisn')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_siswa" id="nama_siswa" class="form-control @error('nama_siswa') is-invalid @enderror" value="{{ old('nama_siswa', $siswa->nama_siswa) }}" placeholder="Masukkan Nama Lengkap Siswa" required maxlength="40">
                        @error('nama_siswa')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select name="jens_kelamin" id="jens_kelamin" class="form-select @error('jens_kelamin') is-invalid @enderror" required>
                            <option value="laki-laki" {{ old('jens_kelamin', $siswa->jens_kelamin) == 'laki-laki' ? 'selected' : '' }}>Laki-Laki</option>
                            <option value="perempuan" {{ old('jens_kelamin', $siswa->jens_kelamin) == 'perempuan' ? 'selected' : '' }}>Perempuan</option>
                        </select>
                        @error('jens_kelamin')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="tahun_masuk" id="tahun_masuk" class="form-control @error('tahun_masuk') is-invalid @enderror" value="{{ old('tahun_masuk', $siswa->tahun_masuk) }}" placeholder="Contoh: 2024" min="2000" max="2099" required>
                        @error('tahun_masuk')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
